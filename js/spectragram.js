...
(function ($, window, document, undefined) {
  ...
  var Instagram = {
    ...
    getRecentMedia: function (userID) {
      var self = this,
        getData = '/users/' + userID + '/media/recent/?' + self.accessData
          .clientID + '&access_token=' + self.accessData.accessToken +
          '';
    ...
    getUserFeed: function () {
      var self = this,
        getData = '/users/search?q=' + self.options.query +
          '&count=' + self.options.max + '&client_id=' + self.accessData
          .clientID + '';
      ...
      self.fetch(getData).done(function (results) {
        if (results.data.length) {
          ...
          $.error('Spectagram.js - Error: the username ' + self.options
    ...
    getPopular: function () {
      var self = this,
        getData = '/media/popular?client_id=' + self.accessData.clientID +
      ...
      if (results.data.length === 0) {
        ...
        for (var i = 0; i < max; i++) {
          ...
          self.$elem.append($(self.options.wrapEachWith).append(
            "<a class='instaVid' data-username='" + results.data[i].user.username + "' data-profile_pic='" + results.data[i].user.profile_picture + "' data-media='" + results.data[i].type + "'title='" + results.data[i].caption.text +
